jQuery(function($){
	var uajax_url = _MBT.uri+"/action/user.php";
	if(_MBT.uru == '1'){
		uajax_url = _MBT.child+"/action/user.php";
	}
  ...
  if($(".user-gridlist.waterfall").length){
	...
	$('.ticket-reply-submit').click(function(){
		...
		if( forms.content.length<5 || forms.content.length>1000 ){
			layer.msg( '回复内容在5-1000字之间' )
	...
	$('body').on('click','.ticket-upload .dripicons-plus',function(){
		...
		$('.ticket-file').change(function(){
	        ...
	        $('body').append('<form id="itemimg-form" style="display:none" enctype="multipart/form-data" action="'+uajax_url+'" method="post"><input type="hidden" name="action" value="ticket.upload"></form>')
	 		...
	 		switch( type ){
            	...
            	break;
	 			...
	 			case 'user.idcard.submit':
		 				...
		 				if( !inputs.verify_name ){
		 					layer.msg('姓名不能为空');
		 					return
		 				}

		 				if( !inputs.verify_idcard ){
		 					layer.msg('身份证不能为空');
		 					return
		 				}

		 				if(confirm("确认信息都填写正确无误？每人只有一次认证机会哦！")){
			 				layer.msg('认证中...')
			 				$.ajax({  
			 					type: 'POST',  
			 					url:  uajax_url,  
								...
								data: {
									action: inputs.action,
									name: inputs.verify_name,
									idcard: inputs.verify_idcard,
									nonce: inputs.verify_nonce
								},
								success: function(data){  
									if( data.error ){
										if( data.msg ){
				        ...
				        time: -1
	 					...
	 					$.ajax({  
	 						...
	 						data: {
	 							...
	 							nonce: $("#erphpcard_nonce").val(),
	 							num: $("#erphpcard_num").val()
	 						},
	 						success: function(data){  
	 							if( data.error ){
	 								if( data.msg ){
	 				...
	 				case 'user.catvip.card.submit':
	 				if(document.getElementById("erphpcard_num2").value==""){
	 					layer.msg("请输入激活码");
	 					return false;
	 				}else{
	 					layer.msg("升级中...", {
				        ...
				        time: -1
	 					...
	 					$.ajax({  
	 						...
	 						dataType: 'json',
	 						data: {
	 							action: 'user.catvip.card',
	 							nonce: $("#erphpcard_nonce2").val(),
	 							num: $("#erphpcard_num2").val()
	 						},
	 						success: function(data){  
	 							if( data.error ){
	 								if( data.msg ){
